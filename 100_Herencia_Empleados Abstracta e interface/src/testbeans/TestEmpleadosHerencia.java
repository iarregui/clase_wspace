...
public class TestEmpleadosHerencia  {
	...
	public static void main(String[] args) {
	  ...
	  // -----------------CARGO LOS EMPLEADOS A ATRAVES DEL METODO CARGAR EMPLEADOS QEU ESTTADEFINIDO MAS ABAJO
		Empleado [] empleados = cargarEmpleados();
		for (Empleado ele: empleados) {
			System.out.println(ele.getClass()+ " "+ele.nombreCompleto() + "  " + ele.totalSalario());
	/*NOTA:  // El metodo getClass devuelve la clase en la que se creo el objeto.
	 * POLIMORFISMO
	 * cuando ejecuto elmetodo total salario . ejecuta el metodo de cada clase especifica , 
	 * si creo un elemento  new a<empleado totalSalario ejecuta el de la clase padre Empleado
	 * si creo un elemento  new director  totalSalario ejecuta el de la clase Director
	 *si creo un elemento  new gerente totalSalario ejecuta el de la clase Gerente
	 */
			System.out.println(ele.getClass()+ " "+ele.nombreCompleto() + " getSalario  " + ele.getSalario());
	/*
	 * NOTA : Aqui uso el metodo getSalario de empleado y veo el atributo salario de cada Empleado en su clase padre.
	 */
		}
		
		Departamento dep130;
		dep130 = new Departamento(130, "logistica");
		
		Gerente empGerente400;
		empGerente400 = new Gerente(200, "daniel", "travieso", "H", 30_000, 32, 3000, dep130, 60, "sony xperia", 10_000);
		
		System.out.println( " "+ empGerente400.nombreCompleto() + " getSalario  " + empGerente400.getSalario()+" totyal salarrio "+empGerente400.totalSalario());
		
		
		//----------------------------------EJEMPLO INSTANCEOF Y CASTING
		
		for (Empleado ele: empleados) {
			
			if (ele instanceof Gerente)// ejemplo de casting
			 System.out.println("Gerente tipo movil"+ ((Gerente)ele).getMovil());
			else if (ele instanceof Director)// ejemplo de casting
				 System.out.println("acciones del director " + ((Director)ele).getAcciones()); 
			else
			System.out.println(ele.getClass()+ " "+ele.nombreCompleto() + "  " + ele.totalSalario());
	...
	public static Empleado[] cargarEmpleados() {
		...
		emp100 = new Operativo (100, "mario", "vargas", "H", 
						 		30000, 45, 1000, dep10, 10);
		
		emp115 = new Operativo (115, "carlos", "lopez", "H", 
				40000, 35, 2000, dep20 , 15);
